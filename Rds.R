...
rds <- readRDS("C:/Users/Admin/Downloads/Updated Overall Dataset with Product Classification (1).RDS")

rds$`Product - Cat 0` <- rds$`Top.Level.Classification`
rds$`Product - Cat 1` <- rds$`Classification_Level1`
rds$`Product - Cat 2` <- rds$`Classification_Level2`
rds$`Product - Cat 3` <- rds$`Classification_Level3`

drops <- c("Top.Level.Classification","Classification_Level1","Classification_Level2","Classification_Level3")
rds <- rds[ , !(names(rds) %in% drops)]
...
rds$`Product - Cat 0`[rds$`Product - Cat 0` == 'Non-Alcoholic Beverage'] <- 'NA Beverage'


rds <- rds %>% filter(!is.na(`item_name`))

rds <- rds %>% filter(!grepl('open staffing|test pr|press 10|open cash drawer|fire backup|null', item_name))



df1 <- rds %>% filter(is.na(`Product - Cat 0`)) %>% 
  mutate(
    `Product - Cat 0` = case_when(
      str_detect(Retail.y,'[Yy]es|YES') ~ 'Retail',
      source_system == 'netsuite' ~ 'Retail',
      str_detect(item_name,'Koozie|[Dd]onation|DONATION') ~ 'Retail',
      str_detect(item_name,retailpattern) ~ 'Retail',
...
unq1<-unique(food_df[c("item_name")])


#write.csv(rds, 'RDS.csv')
...
snackpattern <- 'Ruffles|[cC]heezit|Cheez It|cheez-it|[Rr]eese|m&m|Nacos|Funyuns|Mints|Nestle|Sticks|Egg Rolls|Cheese|Tots|[Tt]oast|[Bb]un|[Dd]ough|[Dd]ulce|[Rr]ose|[Nn]utella|Bar|Baked OriginalNacos|Bacon|Yummies|Protein Bar|Snack|Oreo|Flipz|[Nn]acho|Muffin|[Cc]hip|CHIP|[Pp]eanuts|COOKIES|[Pp]opcorn|Pretzel|Dorito|[Cc]heetos|Cheez-it|Ming\'s Bings|Gourmet Nut Mix|Ruffles|Cheez It|501 Club|Chili|Gardettos|Cheeto|Lay\'s|Waffle|Corn|Snack|Cheez It|Baked Original|Yummies|Protein Bar|Snack|Oreo|Flipz|[Nn]acho|Muffin|[Cc]hip|CHIP|[Pp]eanuts|[Pp]opcorn|Pretzel|Corn|Combo|Smartfood|Dorito|Kind Bar|Trail Mix|Chex Mix|[O]reo'
...
mergedDF <- rbind(fdf,adf,ndf,rdf)

keeps <- c("item_name", "Product - Cat 0","Product - Cat 1","Product - Cat 2","Product - Cat 3")
mergedDF<-mergedDF[keeps]


#rds$`Product - Cat 0` <- ifelse(rds$item_name == mergedDF$item_name, mergedDF$`Product - Cat 0`, rds$`Product - Cat 0`)

sum(is.na(rds$`Product - Cat 0`))


for(i in 1:nrow(rds)) {       # for-loop over rows
  
  if(is.na(rds$`Product - Cat 0`[i])){
    rds$`Product - Cat 0`[i] <- mergedDF$`Product - Cat 0`[mergedDF$item_name ==rds$item_name[i]]
    rds$`Product - Cat 1`[i] <- mergedDF$`Product - Cat 1`[mergedDF$item_name ==rds$item_name[i]]
    rds$`Product - Cat 2`[i] <- mergedDF$`Product - Cat 2`[mergedDF$item_name ==rds$item_name[i]]
  }
  
}

sum(is.na(rds$`Product - Cat 0`))
sum(is.na(rds$`Product - Cat 1`))
sum(is.na(rds$`Product - Cat 2`))
sum(is.na(rds$`Product - Cat 3`))

naCat1 <- rds[is.na(rds$`Product - Cat 1`),]
unique(naCat1[c("Product - Cat 0")])

naCat2 <- rds[is.na(rds$`Product - Cat 2`),]
unique(naCat1[c("Product - Cat 0")])

rds$`Product - Cat 1` <- ifelse(is.na(rds$`Product - Cat 1`) & rds$`Product - Cat 0`=='Retail', 'Retail',rds$`Product - Cat 1`)
rds$`Product - Cat 2` <- ifelse(is.na(rds$`Product - Cat 2`) & rds$`Product - Cat 0`=='Retail', 'Retail',rds$`Product - Cat 2`)
...
rds <- rds %>%
  mutate(
    `Product - Cat 3` = case_when(
      ...
      str_detect(item_name,ozpattern) ~ str_extract(rds$item_name,"[\\d\\.]+[Oo][Zz]|[\\d\\.]+ [Oo][Zz]"),
      str_detect(item_name,mlpattern) ~ str_extract(rds$item_name,"[\\d\\.]+[Mm][Ll]|[\\d\\.]+ [Mm][Ll]"),
      TRUE ~ 'Regular'))
...
rds$`Product - Cat 3`[rds$`Product - Cat 3` == '12Oz' | rds$`Product - Cat 3` == '12oz' | rds$`Product - Cat 3` == '12 oz'] <- '12 Ounces'
rds$`Product - Cat 3`[rds$`Product - Cat 3` == '9Oz' | rds$`Product - Cat 3` == '9oz' | rds$`Product - Cat 3` == '9 oz'] <- '9 Ounces'
rds$`Product - Cat 3`[rds$`Product - Cat 3` == '6Oz' | rds$`Product - Cat 3` == '6oz' | rds$`Product - Cat 3` == '6 oz'] <- '6 Ounces'
rds$`Product - Cat 3`[rds$`Product - Cat 3` == '16 Oz' | rds$`Product - Cat 3` == '16OZ' |rds$`Product - Cat 3` =='16Oz' | rds$`Product - Cat 3` == '16oz' | rds$`Product - Cat 3` == '16 oz'] <- '16 Ounces'
rds$`Product - Cat 3`[rds$`Product - Cat 3` == '25 Oz' | rds$`Product - Cat 3` == '25OZ' |rds$`Product - Cat 3` == '25Oz' | rds$`Product - Cat 3` == '25oz' | rds$`Product - Cat 3` == '25 oz'] <- '25 Ounces'
rds$`Product - Cat 3`[rds$`Product - Cat 3` == '24 Oz' | rds$`Product - Cat 3` == '24OZ' |rds$`Product - Cat 3` == '24Oz' | rds$`Product - Cat 3` == '24oz' | rds$`Product - Cat 3` == '24 oz'] <- '24 Ounces'
rds$`Product - Cat 3`[rds$`Product - Cat 3` == '20 Oz' | rds$`Product - Cat 3` == '20OZ' |rds$`Product - Cat 3` == '20Oz' | rds$`Product - Cat 3` == '20oz' | rds$`Product - Cat 3` == '20 oz'] <- '20 Ounces'
rds$`Product - Cat 3`[rds$`Product - Cat 3` == '19.2 Oz' | rds$`Product - Cat 3` == '19.2OZ' |rds$`Product - Cat 3` == '19.2Oz' | rds$`Product - Cat 3` == '19.2oz' | rds$`Product - Cat 3` == '19.2 oz'] <- '19.2 Ounces'
rds$`Product - Cat 3`[rds$`Product - Cat 3` == '14.9 Oz' | rds$`Product - Cat 3` == '14.9OZ' |rds$`Product - Cat 3` == '14.9Oz' | rds$`Product - Cat 3` == '14.9oz' | rds$`Product - Cat 3` == '14.9 oz'] <- '14.9 Ounces'

rds$`Product - Cat 3`[rds$`Product - Cat 3` == '500ML' | rds$`Product - Cat 3` == '500ml' | rds$`Product - Cat 3` == '500 ML'] <- '500 ml'
rds$`Product - Cat 3`[rds$`Product - Cat 3` == '375ML' | rds$`Product - Cat 3` == '375ml' | rds$`Product - Cat 3` == '375 ML'] <- '375 ml'

rds %>% ggplot() +
  geom_bar(mapping=aes(x=`Product - Cat 3`
  )) + coord_flip()

write.csv(rds, 'MergedFromRDS.csv') 


#rds$`Product - Cat 0`[rds$item_name == mergedDF$item_name] <- mergedDF$`Product - Cat 0`
#rm(finalDF)
#finalDF <- left_join(rds, mergedDF, by = "item_name")
